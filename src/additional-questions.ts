...
export const additionalQuestions: Question[] = [
  ...
  },
  {
    ...
    prompt: "面接官: プログラムがファイルを開いたときに得られる「ファイルディスクリプタ」とは何ですか?",
    correct: { id: "correct", text: "プロセスごとに管理される、開いているリソースを指す小さな整数の識別子" },
    distractors: [
    ...
    brief: "ファイルディスクリプタは、OSがプロセスに割り当てる「開いているリソースへの手札番号」のようなものです。",
    interview: "open()のようなシステムコールを呼ぶと、OSはそのプロセス専用のテーブルにエントリを作り、小さな整数を返します。以降のread/writeはこの番号を介して行われ、実際のファイルやソケットの状態はOS側で管理されます。ソケットやパイプも同じ仕組みで扱われます。",
    relevance: "Webサーバーが「too many open files」エラーを出す場面や、ソケットやDB接続のリークを調査するとき、ディスクリプタの上限や解放漏れの理解が役立ちます。",
  ...
  },
  {
    ...
    prompt: "面接官: メモリマップドファイル（mmap）を使うと、通常のread/write呼び出しと比べて何が変わりますか?",
    correct: { id: "correct", text: "ファイルの内容を仮想メモリ空間に直接マッピングし、メモリアクセスとして読み書きできるようになる" },
    distractors: [
      { id: "d1", text: "ファイルの内容が必ずディスクからメモリへ全量コピーされ、以後は完全に独立したコピーになる" },
    ...
    brief: "mmapはファイルをプロセスの仮想アドレス空間に貼り付け、通常のメモリアクセスとして扱えるようにする仕組みです。",
    interview: "read/writeはシステムコールのたびにカーネルとユーザー空間の間でデータをコピーしますが、mmapはページ単位でファイルを仮想メモリにマッピングし、アクセス時にページフォルトで必要な部分だけ読み込みます。これにより大きなファイルの部分アクセスや複数プロセス間の共有が効率化されます。",
    relevance: "大きなログファイルの解析や、DBエンジンがデータファイルを扱う実装、共有メモリでのプロセス間通信を理解する際にmmapの考え方が出てきます。",
  ...
  },
  {
    ...
    prompt: "面接官: fork()でプロセスを複製するとき、コピーオンライト(copy-on-write)はどのように働きますか?",
    correct: { id: "correct", text: "親子プロセスは最初メモリページを共有し、どちらかが書き込もうとした時点で初めてそのページが複製される" },
    distractors: [
    ...
    brief: "copy-on-writeは、複製を「必要になるまで」先延ばしにして無駄なコピーを避ける最適化です。",
    interview: "fork()直後、親子プロセスは同じ物理ページを指す形で仮想メモリを共有し、ページテーブルは書き込み禁止として設定されます。どちらかのプロセスがそのページに書き込もうとすると、ページフォルトが発生し、その時点で初めて物理ページが複製されて分離されます。読み取りだけなら複製は起きません。",
    relevance: "大きなプロセスをforkしてサブプロセスを起動するサーバーの起動コストや、Redisのようなミドルウェアがスナップショット取得時にfork+copy-on-writeを使う仕組みを理解する上で重要です。",
  ...
  },
  {
    ...
    prompt: "面接官: マルチコアCPUにおける「メモリバリア(メモリフェンス)」は何のために使われますか?",
    correct: { id: "correct", text: "CPUやコンパイラによる命令の並び替えを制限し、コア間でメモリ操作の順序が期待通りに見えるよう保証するため" },
    distractors: [
    ...
    brief: "メモリバリアは、最適化のための命令並び替えが「他のコアから見える順序」を壊さないように歯止めをかける仕組みです。",
    interview: "CPUやコンパイラは性能のために命令の実行順序を並び替えることがありますが、単一コア内では問題なくても、複数コアが同じメモリを見ているとその並び替えが他コアから見た順序の不整合として現れます。メモリバリアは『このバリアより前の書き込みは後の書き込みより先に他コアから見える』といった順序を保証します。ロックフリーアルゴリズムやアトミック変数の実装で明示的、あるいは暗黙的に使われます。",
    relevance: "高性能なロックフリーキューやアトミック操作を扱うライブラリの内部実装や、なぜvolatileだけではスレッド安全にならないかを説明する際に、メモリバリアの理解が必要になります。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "各コアが独自のキャッシュに同じメモリアドレスの値を保持しているとき、それらの値の一貫性をどう保つかという問題" },
    distractors: [
    ...
    brief: "キャッシュコヒーレンシは、複数コアが持つ同じアドレスのキャッシュコピー同士をどう一致させるかという問題です。",
    interview: "各コアはL1/L2キャッシュを個別に持つため、同じメモリアドレスの値が複数のキャッシュに別々にコピーされ得ます。あるコアがその値を書き換えたとき、他コアが古い値を見続けてしまうと不整合が起きます。これを防ぐためMESIプロトコルのような仕組みで、あるキャッシュラインが書き換えられたら他コアのコピーを無効化したり更新したりして一貫性を保ちます。",
    relevance: "false sharingと呼ばれる、隣接する変数を別コアが頻繁に書き換えて性能が落ちる現象や、ロックフリーデータ構造の性能特性を説明する上でキャッシュコヒーレンシの理解が前提になります。",
  ...
  },
  {
    ...
    prompt: "面接官: ロックフリーなデータ構造を実装する際、CAS(Compare-And-Swap)命令はどのような役割を果たしますか?",
    correct: { id: "correct", text: "ある変数の値が期待値と一致する場合にのみ、アトミックに新しい値へ更新することで、ロックなしで安全な状態遷移を可能にする" },
    distractors: [
    ...
    brief: "CASは「期待していた値のままなら書き換える」をアトミックに行うことで、排他ロックなしの安全な更新を実現する基本命令です。",
    interview: "ロックフリーアルゴリズムでは、複数スレッドが同じ共有変数を読み、その読んだ値をもとに新しい値を計算し、CASで『まだその値のままなら更新』を試みます。他スレッドが先に値を変えていた場合はCASが失敗し、再度読み直してリトライします。ロックを取らないためデッドロックが起きにくい一方、競合が激しいとリトライが増え性能が落ちることもあります。",
    relevance: "並行キューやカウンタなど高スループットが求められるライブラリの内部実装、あるいはAtomicIntegerやatomic<T>のようなプリミティブの動作原理を説明する際に必要になります。",
  ...
  },
  {
    ...
    prompt: "面接官: NUMA(Non-Uniform Memory Access)構成のマシンで性能を考える際、何に注意する必要がありますか?",
    correct: { id: "correct", text: "CPUがどのメモリノードにアクセスするかによってレイテンシが異なるため、スレッドとメモリの配置を近づける必要がある" },
    distractors: [
      { id: "d1", text: "すべてのメモリアクセスが常に同じ速度になるよう設計されているため、特別な配慮は不要である" },
    ...
    brief: "NUMAでは『どのメモリがどのCPUに近いか』が性能を左右するため、局所性を意識した配置が重要になります。",
    interview: "NUMA構成では、CPUソケットごとにローカルなメモリノードがあり、自分のノードのメモリへのアクセスは高速ですが、他ソケットのノードへのアクセスはインターコネクトを経由するため遅くなります。スレッドが実行されるコアと、そのスレッドが使うメモリが別ノードにまたがると、レイテンシが増加し性能が劣化します。OSやランタイムはスレッドとメモリの割り当てをできるだけ同じノードに揃えることで、この影響を抑えようとします。",
    relevance: "大規模なDBサーバーやJVMのようなランタイムをマルチソケットマシンでチューニングする際、NUMAを意識したスレッドアフィニティやメモリ割り当ての設定が性能に直結します。",
  ...
  },
  {
    ...
    prompt: "面接官: CPUの分岐予測(branch prediction)を外した場合、性能にどのような影響がありますか?",
    correct: {
      id: "correct",
    ...
    brief: "分岐予測はif文などの分岐先を先読みしてパイプラインを埋める最適化で、外すとやり直しコストがかかります。",
    interview:
  ...
  },
  {
    ...
    brief: "DRY(Don't Repeat Yourself)は、同じ知識を一箇所にまとめ、変更時の修正漏れを防ぐ原則です。",
  ...
  },
  {
    ...
    interview: "「将来必要になりそうだから」と過剰に汎用化すると、複雑さだけが増えることがあります。",
  ...
  },
  {
    ...
    prompt: "面接官: レイヤードアーキテクチャで上位層が下位層の実装詳細に依存しないようにする一般的な手法は?",
    ...
    brief: "インターフェースへの依存により、上位層は下位層の具体的な実装を知らずに済み、疎結合になります。",
    interview: "DBアクセス層をインターフェース越しに呼べば、実装をMySQLからPostgreSQLに変えても上位層は変更不要です。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "拡張に対して開き、既存コードの修正に対して閉じているべきという原則" },
    distractors: [
    ...
    brief: "既存コードを変更せずに、新しい振る舞いを追加(拡張)できるように設計すべきという原則です。",
    interview: "新しい支払い方法を追加する際、既存のif分岐を書き換えず、新しいクラスを追加するだけで済む設計が理想です。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "データの更新(コマンド)と参照(クエリ)のモデル・経路を分離すること" },
    distractors: [
    ...
    brief: "CQRSは更新用モデルと参照用モデルを分けることで、それぞれを独立に最適化できるようにする設計手法です。",
    interview: "参照が多いシステムでは、更新用の正規化されたDBとは別に、参照専用の非正規化ビューを用意することがあります。",
  ...
  },
  {
    ...
    brief: "フィーチャーフラグは機能のON/OFFを設定で切り替え可能にし、デプロイとリリースタイミングを分離する仕組みです。",
    interview: "カナリアリリースやA/Bテストで、一部ユーザーだけに新機能を段階的に公開する際に使われます。",
  ...
  },
  {
    ...
    brief: "バックプレッシャーは、受け手の処理能力を超えるデータ流入を抑えるためのフロー制御メカニズムです。",
    interview: "メッセージキューの消費が追いつかない場合、生産側にシグナルを送って送信レートを落とすことがあります。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "障害中の依存先への呼び出しを一時的に遮断し、カスケード障害を防ぐこと" },
    distractors: [
    ...
    brief: "サーキットブレーカーは、依存先の障害を検知すると呼び出しを一定期間遮断し、システム全体への影響拡大を防ぎます。",
    interview: "決済APIが応答しなくなった際、リトライを繰り返すとスレッドが枯渇するため、一定回数失敗したら回路を開いて即座にエラーを返します。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "リソースを区画分割し、一部の障害が全体に波及しないようにすること" },
    distractors: [
    ...
    brief: "船の隔壁のように、リソース(スレッドプールや接続プールなど)を区画に分け、一部の過負荷が他に波及しないようにする設計です。",
    interview: "ある外部APIの呼び出しが遅延しても、別の重要な処理用のスレッドプールを専用に確保しておけば全体停止を防げます。",
  ...
  },
  {
    ...
    brief: "冪等性キーはクライアントが生成する一意な識別子で、同じキーでの再送を検知して二重処理を防ぐ仕組みです。",
    interview: "ネットワークタイムアウトでクライアントが決済APIを再送しても、サーバー側で同じキーを検知して二重課金を回避します。",
  ...
  },
  {
    ...
    prompt: "面接官: データベースのシャーディング戦略として「レンジベース」と「ハッシュベース」の違いは?",
    correct: { id: "correct", text: "レンジベースはキーの範囲で分割し、ハッシュベースはハッシュ値で分散配置する" },
    distractors: [
    ...
    brief: "レンジベースはキーの範囲ごとにシャードを割り当て、ハッシュベースはキーのハッシュ値で分散させる方式です。",
    interview: "日付範囲で検索が多いデータはレンジベースが有利ですが、特定範囲にアクセスが偏るホットスポットが起きやすい弱点があります。",
  ...
  },
  {
    ...
    brief: "HTTPヘッダーは本文とは別に、Content-TypeやAuthorizationなどのメタ情報を伝える部分です。",
    interview: "リクエストヘッダーとレスポンスヘッダーの両方があり、それぞれ用途の異なる情報を運びます。",
  ...
  },
  {
    ...
    brief: "WebSocketはHTTPのハンドシェイクでコネクションをアップグレードし、双方向通信を実現します。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "1本のTCPコネクション上で複数のリクエストとレスポンスを並行してやり取りできる" },
    distractors: [
    ...
    brief: "HTTP/2はストリームという単位でリクエストを分割し、1本のコネクション上で並行処理します。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "各サーバーの負荷状況に関係なく順番にリクエストを振り分ける方式" },
    distractors: [
    ...
    brief: "ラウンドロビンはシンプルに順番でサーバーへ割り振る方式で、実装が容易な反面、負荷の偏りに弱い場合があります。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "TCPコネクションの確立コストを削減し再利用によって通信を高速化するため" },
    distractors: [
    ...
    brief: "コネクションプーリングは確立済みのTCPコネクションを保持し、再接続のオーバーヘッドを避ける仕組みです。",
    interview: "データベース接続やHTTPクライアントで、毎回の3ウェイハンドシェイクを避けるために使われます。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "多重化やストリーミングなどHTTP/2の機能を利用して双方向の効率的な通信を実現するため" },
    distractors: [
    ...
    brief: "gRPCはProtocol BuffersによるバイナリシリアライズとHTTP/2のストリーム機能を組み合わせています。",
    interview: "サーバーストリーミングやクライアントストリーミングなど、HTTP/2の双方向性を活かした通信パターンが特徴です。",
  ...
  },
  {
    ...
    prompt: "面接官: アクティブ-スタンバイ構成におけるフェイルオーバーの説明として最も適切なものはどれですか?",
    correct: { id: "correct", text: "稼働中のノードに障害が発生した際、待機系のノードへ処理を自動的に切り替える仕組み" },
    distractors: [
    ...
    brief: "フェイルオーバーは障害検知後、自動的にスタンバイ系へ切り替えて可用性を維持する仕組みです。",
  ...
  },
  {
    ...
    domain: "network",
    tags: ["quic", "http3"],
    prompt: "面接官: QUIC(HTTP/3の基盤)がTCPではなくUDP上に構築された理由として最も適切なものはどれですか?",
    correct: { id: "correct", text: "TCPのHead-of-Lineブロッキングを回避し、独自の輻輳制御やコネクション確立の高速化を実現するため" },
    distractors: [
    ...
    brief: "QUICはUDP上にトランスポート層の機能を独自実装し、ストリーム単位でロス回復や暗号化を行います。",
    interview: "QUICはTLS 1.3をプロトコルに統合しており、コネクション確立とハンドシェイクを同時に行えます。",
    relevance: "モバイル環境でのコネクション切り替えや低遅延通信の要件でHTTP/3採用理由を説明できるかが問われます。",
  ...
  },
  {
    ...
    domain: "network",
    tags: ["tcp", "congestion-control"],
    prompt: "面接官: BBRのような輻輳制御アルゴリズムがCUBICと異なる点として最も適切なものはどれですか?",
    correct: { id: "correct", text: "パケットロスではなく帯域幅と往復遅延の推定値を基に送信レートを制御する" },
    distractors: [
      { id: "d1", text: "パケットロスが発生するまで送信レートを増やし続け、ロス検知後に急減させる" },
    ...
    brief: "CUBICはロスベースでウィンドウサイズを増減させるのに対し、BBRは帯域幅とRTTのモデルに基づいて制御します。",
    interview: "高遅延・高帯域のネットワークではロスベースの輻輳制御が非効率になりやすく、BBRのような手法が有利とされます。",
    relevance: "パフォーマンスチューニングやネットワーク基盤の選定で輻輳制御の違いを理解しているかが問われます。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "各ノードがランダムに選んだ他ノードと情報を交換し合い、確率的に全体へ伝播させる仕組み" },
    distractors: [
    ...
    brief: "Gossipプロトコルはノード同士がランダムに通信し合うことで、障害に強く緩やかに情報を伝播させます。",
    interview: "Cassandraなどの分散データベースでメンバーシップ管理や障害検知に利用されています。",
    relevance: "大規模分散システムでのスケーラビリティと耐障害性のトレードオフを説明できるかが問われます。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "複数のエッジロケーションからのキャッシュミスを集約し、オリジンサーバーへのリクエスト数を抑える" },
    distractors: [
    ...
    brief: "オリジンシールドは中間キャッシュ層として機能し、複数エッジからの重複したオリジンリクエストを集約します。",
    interview: "キャッシュミス時に多数のエッジが同時にオリジンへアクセスする「サンダリングハード」問題を緩和します。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "入力値をそのままSQL文に組み込むことで、意図しないSQLを実行させる攻撃" },
    distractors: [
    ...
    interview: "例えばフォームの入力をそのまま文字列結合でSQLに埋め込むと、攻撃者が条件式を書き換えて認証を回避したりデータを抜き出したりできます。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "接続先サーバーの正当性を証明し、通信の暗号化に使う鍵を安全に配布する" },
    distractors: [
    ...
    interview: "証明書には公開鍵とドメイン情報が含まれ、信頼できるCAの署名があることでブラウザが正当なサーバーだと判断できます。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "長く複雑なパスフレーズを許容し、定期的な強制変更よりも多要素認証を重視する" },
    distractors: [
    ...
    interview: "頻繁な変更は使い回しや単純化を招くため、長いパスフレーズと多要素認証(MFA)の組み合わせが推奨されています。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "知識(パスワード)、所持(スマホ・トークン)、生体(指紋など)の組み合わせ" },
    distractors: [
    ...
    interview: "同じ種類の情報を2つ使ってもMFAとは呼ばず、知識・所持・生体など異なるカテゴリを組み合わせることが重要です。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "透明なiframeで正規ページを重ねてクリックを騙す攻撃で、X-Frame-OptionsやCSPのframe-ancestorsで防ぐ" },
    distractors: [
    ...
    interview: "攻撃者は透明なiframeで正規サイトのボタンを重ね、別の見た目のボタンをクリックさせることで意図しない操作を実行させます。X-Frame-OptionsヘッダーやCSPのframe-ancestorsディレクティブでiframe埋め込みを制限するのが基本対策です。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "スクリプトの読み込み元やインライン実行を許可リストで制限し、悪意あるスクリプトの実行を防ぐ" },
    distractors: [
    ...
    interview: "XSSはHTMLエスケープなどの入力対策をすり抜けることがあるため、CSPで許可されたドメイン以外のスクリプトやインラインスクリプトの実行をブロックする多層防御として機能します。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "Access-Control-Allow-OriginをワイルドカードにしたままAllow-Credentialsを有効にすると、任意サイトから認証済みリクエストの結果を読み取られる" },
    distractors: [
    ...
    brief: "CORSはブラウザが実装するオリジン間リクエスト制御の仕組みで、設定ミスが情報漏洩に直結します。",
    interview: "OriginをリクエストヘッダーからそのままAllow-Originに反映しつつAllow-Credentials: trueを付けてしまうと、悪意あるサイトが被害者のCookieを使ったリクエストを送り、レスポンスを読み取れてしまいます。オリジンは許可リストで厳密に検証する必要があります。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "依存パッケージやビルドツールの改ざんを通じた攻撃であり、SBOMは構成要素を可視化し脆弱性の影響範囲を素早く特定するのに役立つ" },
    distractors: [
    ...
    brief: "サプライチェーン攻撃は自組織ではなく依存先や配布経路を狙う攻撃で、影響範囲が広くなりやすいです。",
    interview: "攻撃者は人気パッケージのメンテナ権限を乗っ取ったりビルド環境に侵入したりして悪意あるコードを混入させます。SBOM(Software Bill of Materials)は使用しているライブラリとそのバージョンを一覧化したもので、新たな脆弱性が発表された際に影響範囲を即座に特定できます。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "比較処理の時間差から秘密情報を推測する攻撃で、定数時間比較を使うことで対策する" },
    distractors: [
    ...
    brief: "タイミング攻撃はサイドチャネル攻撃の一種で、処理時間という副次情報から秘密を推測します。",
    interview: "通常の文字列比較は不一致箇所が見つかった時点で処理を打ち切るため、正解に近いほど比較時間がわずかに長くなります。攻撃者はこの微小な時間差を統計的に観測し、APIキーやトークンを1文字ずつ推測できてしまいます。対策として、比較にかかる時間が入力内容によらず一定になる定数時間比較関数を用います。",
  ...
  },
  {
    ...
    domain: "security",
    tags: ["injection"],
    prompt: "面接官: SQLインジェクション以外に、システムが注意すべきインジェクション攻撃の種類を1つ挙げてください。",
    correct: { id: "correct", text: "OSコマンドインジェクション(外部入力をシェルコマンドに渡すことで任意コマンドを実行される攻撃)" },
    distractors: [
    ...
    brief: "インジェクション系の脆弱性はSQLに限らず、外部入力を解釈・実行する箇所全般に存在し得ます。",
    interview: "OSコマンドインジェクションは、ユーザー入力をそのままシェルコマンドの引数に組み込んでしまうことで、攻撃者が「; rm -rf /」のような文字列を注入し任意コマンドを実行できてしまう脆弱性です。他にもLDAPインジェクションやテンプレートインジェクションなど、構造化言語を組み立てる箇所全てが対象になり得ます。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "サーバーだけでなくクライアント側も証明書で認証することで、サービス間通信のなりすましを双方向で防止できる" },
    distractors: [
    ...
    interview: "通常のTLSはクライアントがサーバー証明書を検証して正当性を確認しますが、mTLSではサーバーもクライアント証明書を検証します。これによりサービスメッシュ内で『どのサービスが通信してきたか』を暗号学的に確認でき、不正なサービスやなりすましたクライアントからの通信を遮断できます。",
    relevance: "マイクロサービス基盤やゼロトラストネットワークの設計面接で問われる発展的トピックです。",
  ...
  },
  {
    ...
    correct: { id: "correct", text: "隔離された実行環境の境界の脆弱性を突いて、ホストシステムやより高い権限のリソースにアクセスする攻撃" },
    distractors: [
    ...
    brief: "サンドボックスはコンテナやブラウザ、WASMランタイムなどコードを安全に隔離実行するための仕組みです。",
    interview: "サンドボックスはカーネルのシステムコールフィルタや権限分離によって隔離を実現しますが、その実装自体にバグがあると、攻撃者は隔離境界を破ってホストのファイルシステムや他のプロセスにアクセスできてしまいます。コンテナのカーネル共有アーキテクチャ上、カーネルの脆弱性が直接サンドボックス脱出に繋がる点が特に警戒されます。",
    relevance: "マルチテナント基盤やコード実行サービス(FaaS、CIランナーなど)の設計で重要な脅威モデルです。",
